...
file_path2 = os.path.join(parent_directory, "data", "val_content.json")
with open(file_path1, "r", encoding="utf-8") as c:
    contents = json.load(c)
with open(file_path2, "r", encoding="utf-8") as v:
...
token_train = tokenizer.batch_encode_plus(
    X, max_length=13, padding="max_length", truncation=True
)
...
token_val = tokenizer.batch_encode_plus(
    X_val, max_length=13, padding="max_length", truncation=True
)
...
train_losses = []
valid_losses = []
file_name = os.path.join(current_directory, "PhoBert_weight.pth")
